...
const articles = [
  {
    ...
    excerpt: 'An introduction to Node.js, the JavaScript runtime that powers modern backend development.',
  ...
  },
  {
    ...
    excerpt: 'Learn what REST APIs are, how they work, and the core principles behind them.',
  ...
  },
  {
    ...
    excerpt: 'A simple explanation of how data travels across the internet using networks and protocols.',
  ...
  {
    title: 'Draft Example: Deep Dive into JSON Web Tokens',
    slug: 'draft-example-jwt',
    excerpt: 'A draft article about JWT authentication that is not yet published.',
    content: '<p>This article is currently a draft and will only be visible in the admin panel until it is published.</p>',
...
async function seed() {
  try {
    ...
    const [articleRows] = await conn.query('SELECT COUNT(*) AS total FROM articles');
    if (articleRows[0].total > 0) {
    ...
    const [userRows] = await conn.query('SELECT COUNT(*) AS total FROM users WHERE email = ?', [
      'admin@beyondtoday.com',
    ]);
    ...
    for (const a of articles) {
      const published_at =
        a.status === 'draft'
          ? null
          : new Date(Date.now() - a.days_ago * 24 * 60 * 60 * 1000);
      await conn.query(
